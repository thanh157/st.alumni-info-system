...
@section('title', 'Chỉnh sửa đợt khảo sát thông tin liên hệ')
...
@section('content')
    <div class="container py-4">
        ...
        <div
            ...
            <div>
                <h4 class="fw-bold mb-1">Chỉnh sửa đợt khảo sát - Thông tin liên hệ</h4>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="#">Bảng điều khiển</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('admin.contact-survey.index') }}">Khảo sát liên hệ</a>
                        </li>
        ...
        <!-- Form -->
        <form action="{{ route('admin.contact-survey.update', $survey->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row g-4">
                <!-- Bên trái -->
                    ...
                    <div class="card p-4 shadow-sm h-100">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="title" required
                                value="{{ old('title', $survey->title) }}" {{ $survey->isInActive() ? 'readonly' : '' }}>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Mô tả</label>
                            <textarea name="description" class="form-control" rows="3" {{ $survey->isInActive() ? 'readonly' : '' }}>{{ old('description', $survey->description) }}</textarea>
                        </div>
                ...
                <div class="col-12 col-md-4">
                    <div class="card p-4 shadow-sm h-100">
                        ...
                        <!-- Trạng thái -->
                        <h6 class="mb-3 mt-4">Trạng thái</h6>
                        <div class="mb-3">
                            <select class="form-select" name="status">
                                <option value="1" {{ $survey->status == 1 ? 'selected' : '' }}>Hoạt động</option>
                                <option value="0" {{ $survey->status == 0 ? 'selected' : '' }}>Ẩn</option>
                            </select>
                        </div>
                    </div>
                </div>
    ...
    @push('script')
        <script>
            $(document).ready(function() {
                ...
                // Sự kiện thay đổi năm tốt nghiệp
                $('#school_years').on('change', function() {
                    const selectedYears = $(this).val() ?? [];

                    // Nếu không chọn năm nào → clear luôn đợt tốt nghiệp
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            // Ghi nhớ các đợt đang chọn
                            const currentSelected = $('#graduation_id').val() ?? [];

                            // Làm mới danh sách đợt tốt nghiệp
                            $('#graduation_id').empty();
                ...
                @if ($survey->isInactive())
                    $('#school_years').on('select2:opening', function(e) {
                        e.preventDefault();
                    });
